...
#[cfg(target_os = "linux")]
use crate::openat2::{
    kernel_version, probe_openat2, KernelVersion, MIN_OPENAT2_KERNEL,
    OpenHow, RESOLVE_BENEATH, RESOLVE_NO_MAGICLINKS, RESOLVE_NO_SYMLINKS,
    O_RDONLY, O_WRONLY, O_CREAT, O_EXCL, O_TRUNC, O_APPEND, O_CLOEXEC,
};
...
impl JailFile {
    ...
    /// }
    /// ```
    pub fn has_hard_links(&self) -> bool { self.attestation.nlink > 1 }
}
...
impl std::ops::Deref for JailFile {
    type Target = File;
    fn deref(&self) -> &File { &self.file }
}

impl std::ops::DerefMut for JailFile {
    fn deref_mut(&mut self) -> &mut File { &mut self.file }
}
...
/// [`JailFile::sign_attestation`]. Unsigned attestations are valid for logging
...
impl OpenOptions {
    ...
    pub fn read(mut self, v: bool)       -> Self { self.read = v; self }
    pub fn write(mut self, v: bool)      -> Self { self.write = v; self }
    pub fn append(mut self, v: bool)     -> Self { self.append = v; self }
    pub fn truncate(mut self, v: bool)   -> Self { self.truncate = v; self }
    /// Create the file if it does not exist (requires `write` or `append`).
    pub fn create(mut self, v: bool)     -> Self { self.create = v; self }
    /// Fail if the file already exists (`O_CREAT | O_EXCL`).
    pub fn create_new(mut self, v: bool) -> Self { self.create_new = v; self }
    /// Reject any symlinks inside the jail (Linux: `RESOLVE_NO_SYMLINKS`).
    pub fn no_symlinks(mut self, v: bool) -> Self { self.no_symlinks = v; self }
}
...
mod linux_impl {
    use super::*;
    use crate::openat2::{openat2, Errno, KernelVersion, MIN_OPENAT2_KERNEL,
        kernel_version, probe_openat2,
        OpenHow, RESOLVE_BENEATH, RESOLVE_NO_MAGICLINKS, RESOLVE_NO_SYMLINKS,
        O_RDONLY, O_WRONLY, O_CREAT, O_EXCL, O_TRUNC, O_APPEND, O_CLOEXEC,
    };
    use std::ffi::CString;
    ...
    pub(crate) fn jail_open(
        ...
        let path_str = rel_path.to_str().ok_or_else(|| {
            JailError::InvalidPath("path contains invalid UTF-8".into())
        })?;
        if path_str.contains('\0') {
            return Err(JailError::InvalidPath("null bytes not allowed".into()));
        }
        let cpath = CString::new(path_str).map_err(|_| {
            JailError::InvalidPath("could not convert path to C string".into())
        })?;

        // Build O_* flags
        let mut flags: u64 = O_CLOEXEC;
        if opts.read && !opts.write && !opts.append { flags |= O_RDONLY; }
        if opts.write { flags |= O_WRONLY; }
        if opts.append { flags |= O_APPEND | O_WRONLY; }
        if opts.create { flags |= O_CREAT; }
        if opts.create_new { flags |= O_CREAT | O_EXCL; }
        if opts.truncate { flags |= O_TRUNC; }
        // Default: read-only
        if flags == O_CLOEXEC { flags |= O_RDONLY; }

        // Build RESOLVE_* flags
        let mut resolve = RESOLVE_BENEATH | RESOLVE_NO_MAGICLINKS;
        if opts.no_symlinks { resolve |= RESOLVE_NO_SYMLINKS; }

        let how = OpenHow { flags, mode: 0o666, resolve };

        let owned_fd = openat2(dirfd.as_raw_fd(), &cpath, &how).map_err(|e| {
            map_errno_to_jail_error(e, rel_path)
        })?;
    ...
    pub(crate) fn fstat(fd: i32) -> std::io::Result<StatResult> {
        ...
        Ok(StatResult { dev: s.st_dev, ino: s.st_ino, nlink: s.st_nlink })
    }
...
mod fallback_impl {
    ...
    pub(crate) fn jail_open(
        ...
        if opts.read        { oo.read(true); }
        if opts.write       { oo.write(true); }
        if opts.append      { oo.append(true); }
        if opts.truncate    { oo.truncate(true); }
        if opts.create      { oo.create(true); }
        if opts.create_new  { oo.create_new(true); }
        if !opts.read && !opts.write && !opts.append { oo.read(true); }
...
impl FdJail {
    ...
    pub fn new(root: impl AsRef<Path>) -> Result<Self, JailError> {
        ...
        if root.parent().is_none() || !root.is_dir() {
            return Err(JailError::InvalidJailRoot { path: root, source: std::io::Error::new(
                std::io::ErrorKind::InvalidInput, "not a directory or is filesystem root"
            )});
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            crate::openat2::probe_openat2().map_err(|_| {
                let version = crate::openat2::kernel_version()
            ...
            let dirfd = unsafe {
                std::os::unix::io::OwnedFd::from_raw_fd(
                    std::os::unix::io::IntoRawFd::into_raw_fd(dir_file)
                )
            };
            return Ok(FdJail { root, root_inode: stat.ino, dirfd });
        }
        ...
        #[cfg(not(target_os = "linux"))]
        {
            ...
            use std::os::unix::fs::MetadataExt;
            Ok(FdJail { root, root_inode: meta.ino() })
        }
    }
